...
class FormList {
            ...
            */
            ...
            "columns": [
                { "data": "name", "name": "name", "title": "name", mRender: (data, type, row) => {
                    debugger;
            return `
                        ${row['name']} <span class="c-table__edit-button"><a href="${Routing.generate('editor_edit_form', {'uid' : row['uid'], 'internalIdentifier' : this.portal})}" data-bypass="true" class="btn btn-primary btn-sm">Edit</a></span>
                        <span class="js-delete-form c-table__delete-button" data-uid="${row['uid']}"></span>
                    `;
                }},
                { "data": "submissionCount", "name": "submissionCount", "title": "Submissions"}
            ],
            "ajax": {
                url: Routing.generate('forms_for_datatable', {internalIdentifier: this.portal}),
                type: "GET",
                dataType: "json",
    ...
    render() {
        return new Promise((resolve, reject) => {
            this.$wrapper.html(FormList.markup(this));
            resolve();
        });
    }
